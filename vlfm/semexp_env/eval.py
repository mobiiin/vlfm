...
def main() -> None:
    ...
    # Initialize the policy with the first dynamic prompt
    initial_prompt = prompt_engineer.generate_prompt({})
    policy_kwargs = dict(
        text_prompt=initial_prompt,  # Use the dynamically generated prompt
    ...
    if exp_thresh > 0.0:
        ...
        policy_kwargs["text_prompt"] = initial_prompt  # Use the same initial prompt for V3
    else:
    ...
    for ep_num in range(num_episodes):
        ...
        for step in range(args.max_episode_length):
            ...
            if "ZSOS_LOG_DIR" in os.environ and is_evaluated(ep_id, scene_id):
                ...
                parsed_response = policy.parse_response(policy_infos.get("response", ""))  # Parse the model's response
                new_prompt = prompt_engineer.generate_prompt(parsed_response)  # Generate a new prompt
                policy.text_prompt = new_prompt  # Update the policy's prompt
...
def display_tiled_frames(frames: list, window_name: str = "Evaluation Frames"):
    """
    Concatenate and display frames in a 2x2 tile format.
    
    Args:
        frames (list): List of 4 frames (numpy arrays) to display.
        window_name (str): Name of the display window.
    """
    # Ensure we have exactly 4 frames to tile
    if len(frames) != 4:
        raise ValueError("Exactly 4 frames are required for a 2x2 tile display.")
    
    # Resize all frames to match dimensions (assume they have the same aspect ratio)
