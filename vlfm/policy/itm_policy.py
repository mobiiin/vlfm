...
class BaseITMPolicy(BaseObjectNavPolicy):
    ...
    def _propose_additional_frontiers(self, frontiers: np.ndarray) -> np.ndarray:
        ...
        value_map_data = self._value_map  # Shape: (size, size, value_channels)
        ...
        high_value_points = self._indices_to_world(high_value_indices)
    ...
    def _indices_to_world(self, indices: np.ndarray) -> np.ndarray:
        """
        ...
        """
        # Convert indices to world coordinates
        world_points = (indices - self._episode_pixel_origin) / self.pixels_per_meter
