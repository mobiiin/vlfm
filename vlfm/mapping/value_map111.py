...
class ValueMap(BaseMap):
    ...
    def _create_circular_mask(self, center: Tuple[int, int], radius: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        Create a circular mask centered at the robot's position, divided into 4 quadrants.

        Args:
            center: The (x, y) coordinates of the robot's position in the map.
            radius: The radius of the circle in pixels.

        Returns:
            Four boolean masks for the forward, backward, left, and right quadrants.
        """
        y, x = np.ogrid[-center[0]:self.size - center[0], -center[1]:self.size - center[1]]
        mask = x**2 + y**2 <= radius**2  # Circular mask

        # Divide the circle into 4 non-overlapping quadrants
        forward_mask = np.logical_and(mask, np.logical_and(x >= 0, np.abs(y) <= np.abs(x)))  # Forward quadrant
        backward_mask = np.logical_and(mask, np.logical_and(x < 0, np.abs(y) <= np.abs(x)))  # Backward quadrant
        left_mask = np.logical_and(mask, np.logical_and(y >= 0, np.abs(y) > np.abs(x)))  # Left quadrant
        right_mask = np.logical_and(mask, np.logical_and(y < 0, np.abs(y) > np.abs(x)))  # Right quadrant
    ...
    def update_map(
        ...
        # Calculate the radius of the circle based on max_depth and FOV
        ...
        # self._value_map[right_mask, 0] = values[3]  # "Turn right"
        ...
        # Localize the new data (depth and FOV) into the map
        ...
        self._fuse_new_data(curr_map, values, masks)
    ...
    def _fuse_new_data(
        ...
        assert (
            len(values) == 4
        ), f"Incorrect number of values given ({len(values)}). Expected {4}."
        ...
        if self._use_max_confidence:
            ...
            self._value_map = self._value_map * weight_1_channeled + values[0] * weight_2_channeled
